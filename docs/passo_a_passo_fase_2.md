...
O modelo de embeddings é carregado uma única vez por processo de worker e reutilizado durante todo o ciclo de vida da aplicação, evitando múltiplas inicializações custosas e mantendo o serviço stateless do ponto de vista externo.
...
3.  **Tagging da Fase 2 (Marco Temporal):**
    Como o bump2version está configurado apenas para atualizar arquivos (sem criar tags globais), crie a tag manualmente:
    ```bash
    git tag v0.2.0
    ```
