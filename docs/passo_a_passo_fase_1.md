...
## 1.1 Estratégia de Ingestão: Falhas, Retry, Idempotência e Observabilidade

Esta seção descreve como o Ingestion Service se comporta em cenários adversos, garantindo robustez operacional, idempotência e capacidade de diagnóstico em produção.

### 1.1.1 Princípios Operacionais

O serviço de ingestão foi projetado segundo os seguintes princípios:

*   **Falhas são esperadas, não exceções.**
*   O serviço pode ser reexecutado múltiplas vezes sem gerar inconsistências.
*   Toda operação relevante deve ser observável por logs e métricas.
*   Erros externos não devem comprometer o estado interno do sistema.

### 1.1.2 Estratégia de Idempotência

Cada artigo ingerido é identificado por uma chave única e determinística:

`<source>/<article_id>.json`

Essa chave é utilizada como nome do objeto persistido na camada Bronze (S3/MinIO).

**Consequências práticas:**

*   Reexecuções sobrescrevem o mesmo objeto.
*   Não ocorre duplicação de dados.
*   O serviço pode ser interrompido e reiniciado com segurança.
*   Opcionalmente, o serviço pode verificar a existência prévia do objeto antes da escrita, otimizando custos de I/O.

### 1.1.3 Classificação de Falhas e Comportamento Esperado

| Tipo de Falha | Exemplo | Comportamento |
| :--- | :--- | :--- |
| Timeout de rede | HTTP timeout | Retry com backoff exponencial |
| Erro HTTP 4xx | 404, 403 | Falha definitiva, log e skip |
| Erro HTTP 5xx | 500, 503 | Retry |
| Erro de parsing | HTML inválido | Log estruturado e skip |
| Falha de persistência | Erro S3 | Retry |
| Artigo duplicado | Mesmo article_id | Operação idempotente |

> [!NOTE]
> Falhas definitivas não interrompem o batch completo, apenas o item afetado.

### 1.1.4 Política de Retry

A política de retry segue o padrão:

*   `max_retries`: 3
*   `backoff`: exponencial (2s, 4s, 8s)
*   `jitter`: habilitado

Essa abordagem reduz risco de thundering herd e bloqueios por rate limit.

### 1.1.5 Observabilidade

**Logs Estruturados**

Todos os eventos relevantes são registrados em formato estruturado (JSON).

Exemplo de evento bem-sucedido:
```json
{
  "event": "article_ingested",
  "source": "arxiv",
  "article_id": "2401.12345",
  "duration_ms": 842,
  "status": "success"
}
```
...
```json
{
  "event": "article_ingestion_failed",
  "source": "arxiv",
  "article_id": "2401.12345",
  "error_type": "timeout",
  "retry_attempt": 2,
...
Todos os logs incluem um `correlation_id` para futura integração com tracing distribuído.

**Métricas (Conceituais)**

Mesmo em MVP, o serviço expõe ou documenta métricas essenciais:

| Métrica | Tipo | Descrição |
| :--- | :--- | :--- |
| `ingestion_articles_total` | Counter | Artigos ingeridos |
| `ingestion_failures_total` | Counter | Falhas por tipo |
| `ingestion_duration_seconds` | Histogram | Latência por artigo |
| `ingestion_retries_total` | Counter | Número de retries |

### 1.1.6 Impacto no Pipeline

Essas garantias permitem que:

*   A Fase 2 (Processing Service) consuma dados com contratos estáveis.
*   Reprocessamentos completos do dataset sejam seguros.
*   Incidentes sejam diagnosticados rapidamente.
*   O serviço opere de forma autônoma em produção.

---

## 1.2 Contrato de Dados da Camada Bronze

Esta seção define o contrato formal de dados persistidos pelo Ingestion Service na camada Bronze do Data Lake.
O objetivo é garantir interoperabilidade, idempotência e evolução controlada entre os serviços do pipeline.

### 1.2.1 Princípios do Contrato

O contrato da camada Bronze segue os princípios abaixo:

*   A Bronze armazena dados brutos, porém estruturados.
*   Nenhuma normalização semântica é aplicada nesta camada.
*   O schema é estável e versionado.
*   Mudanças incompatíveis exigem nova versão de schema.
*   Serviços consumidores não devem assumir campos opcionais como obrigatórios.

### 1.2.2 Organização dos Objetos no Storage

Os objetos são persistidos no S3/MinIO segundo a seguinte convenção:

`bronze/<source>/<article_id>.json`

Exemplo:

`bronze/arxiv/2401.12345.json`
...
### 1.2.3 Versão do Schema

Todo objeto persistido inclui explicitamente a versão do schema:

`"schema_version": "bronze-1.0"`

Regras de versionamento:

*   **Alterações compatíveis:** incremento minor (`bronze-1.1`)
*   **Alterações incompatíveis:** incremento major (`bronze-2.0`)
*   Serviços consumidores devem validar a versão antes do processamento

### 1.2.4 Estrutura do Documento

#### 1.2.4.1 Estrutura Geral
```json
{
  "schema_version": "bronze-1.0",
  "ingestion_metadata": { ... },
...
#### 1.2.4.2 ingestion_metadata

Metadados técnicos relacionados à ingestão.

```json
{
  "source": "arxiv",
  "ingested_at": "2026-01-05T14:32:10Z",
...
| :--- | :--- | :--- | :--- |
| `source` | string | Sim | Fonte dos dados |
| `ingested_at` | string (ISO 8601) | Sim | Timestamp da ingestão |
| `service` | string | Sim | Nome do serviço |
| `service_version` | string | Sim | Versão do serviço |
| `correlation_id` | string | Sim | Identificador para tracing |

#### 1.2.4.3 article

Dados brutos do artigo conforme disponibilizados pela fonte.

```json
{
  "id": "2401.12345",
  "title": "Large Language Models for X",
  "authors": [
    "John Doe",
    "Jane Smith"
  ],
  ...
  "updated": "2024-01-05",
  "links": {
    "abstract": "https://arxiv.org/abs/2401.12345",
...
| :--- | :--- | :--- | :--- |
| `id` | string | Sim | Identificador único do artigo |
| `title` | string | Sim | Título conforme a fonte |
| `authors` | array[string] | Não | Ordem preservada |
| `summary` | string | Não | Texto bruto |
| `categories` | array[string] | Não | Sem normalização |
| `published` | string | Não | Data original |
| `updated` | string | Não | Última atualização |
| `links` | object | Não | URLs originais |
| `raw` | object | Não | Conteúdo bruto opcional |

### 1.2.5 Campos Opcionais e Tolerância a Ausência

*   Campos opcionais podem não existir ou estar vazios.
*   Serviços consumidores devem assumir ausência como comportamento normal.
*   Nenhum campo opcional deve ser inferido implicitamente.

### 1.2.6 Garantias do Contrato

A camada Bronze garante que:

*   Cada objeto representa uma única entidade da fonte.
*   O documento é autossuficiente.
*   O conteúdo pode ser reprocessado integralmente.
*   O schema é consistente dentro da mesma versão.

### 1.2.7 Responsabilidades dos Serviços Consumidores

Serviços da Fase 2 (Processing) devem:

*   Validar `schema_version`
*   Nunca alterar dados da Bronze
*   Tratar ausência de campos como esperado
*   Gerar novos artefatos em camadas superiores

### 1.2.8 Evolução do Contrato

A evolução do contrato segue estas diretrizes:

*   Alterações são documentadas neste arquivo
*   Mudanças incompatíveis geram nova versão
*   Múltiplas versões podem coexistir no storage
*   Migrações são responsabilidade de jobs dedicados
...
Crie o arquivo `.bumpversion.cfg` na raiz do `ingestion_service` para configurar como o bump2version deve se comportar.
Este arquivo diz à ferramenta quais arquivos contêm a string de versão que precisa ser atualizada (neste caso, apenas o `pyproject.toml`).
...
# Tagging da Fase 1 (Marco Temporal)
git tag v0.1.0
